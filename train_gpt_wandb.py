...
if __name__ == '__main__':
    # ==================== 初始化 wandb ====================
    wandb.init(
        project="gpt-training",  # 项目名称
        name=f"gpt-d{GPT_DIM}-h{GPT_HEAD}",  # 运行名称
        config={
            "d_model": GPT_DIM,
            "nhead": GPT_HEAD,
            "feedforward": GPT_FF,
    ...
    for i in range(start_iter, TRAIN_ITER):
        ...
        # 每步都记录即时loss和学习率到 wandb
        wandb.log({
            "loss/step": loss.item() * accum_steps,
        ...
        if (i + 1) % sample_interval == 0:
            ...
            # 记录生成样本到 wandb
            wandb.log({
                "samples": wandb.Html(f"<pre>{sample_text}</pre>")
            }, step=i+1)
    ...
    # 保存最终模型到 wandb
    wandb.save('final_checkpoint.bin')
    
    # 记录最终指标
    wandb.log({
        "final/best_loss": best_loss,
        "final/total_steps": TRAIN_ITER,
    })
